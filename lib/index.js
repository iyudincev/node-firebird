...
SQLVarDate.prototype.decode = function(data) {
    ...
    if (!data.readInt()) {
        var d = new Date(0);
        d.setMilliseconds((ret - DateOffset) * TimeCoeff + getTimeOffset(d));
        return d;
    }
    return null;
};
...
SQLVarTime.prototype.decode = function(data) {
    ...
    if (!data.readInt()) {
        var d = new Date(0);
        d.setMilliseconds(Math.floor(ret / 10) + getTimeOffset(d));
        return d;
    }
    return null;
};
...
SQLVarTimeStamp.prototype.decode = function(data) {
    ...
    if (!data.readInt()) {
        var d = new Date(0);
        d.setMilliseconds((date - DateOffset) * TimeCoeff + Math.floor(time / 10) + getTimeOffset(d));
        return d;
    }
    return null;
};
...
Connection.prototype.connect = function (database, callback) {
    ...
    msg.addString(database || '', this.options.fsEncoding || DEFAULT_ENCODING);
...
Connection.prototype.attach = function (options, callback, db) {
    ...
    var characterSet = options.characterSet || DEFAULT_ENCODING;
    ...
    blr.addString(isc_dpb_lc_ctype, characterSet, DEFAULT_ENCODING);
...
Connection.prototype.createDatabase = function (options, callback) {
    ...
    //blr.addString(isc_dpb_lc_ctype, 'UTF8', DEFAULT_ENCODING);
    blr.addString(isc_dpb_set_db_charset, characterSet, DEFAULT_ENCODING);
    blr.addString(isc_dpb_lc_ctype, characterSet, DEFAULT_ENCODING);
    blr.addString(isc_dpb_user_name, user, DEFAULT_ENCODING);
    blr.addString(isc_dpb_password, password, DEFAULT_ENCODING);
